...
| **批量重命名** | 组合替换 / 正则、插入、删除、大小写、编号、模板（日期与 EXIF 拍摄时间）等规则批量改名，实时预览并检查冲突，两阶段安全改名并可撤销 |
...
| **SSH 密钥** | 生成 OpenSSH 格式的 Ed25519、ECDSA、RSA 密钥并可设置口令；查看密钥、authorized_keys 与 known_hosts 的 SHA256 / MD5 指纹和随机图（与 ssh-keygen 一致），转换 PEM 格式 RSA 私钥，修改口令 |
...
| **WebSocket 客户端** | 连接 ws:// 与 wss:// 服务，可设置请求头与子协议，实时查看收发的每一帧并格式化 JSON，发送文本、十六进制 / Base64 二进制或 Ping，支持保存常用片段 |
...
| **磁盘空间分析** | 并行扫描文件夹或磁盘，用按文件类型着色、可逐级进入的树图查看空间占用，列出文件夹内容、最大文件与类型统计，并可移到回收站 |
| **重复文件查找** | 跨文件夹查找内容完全相同的文件（大小 → 开头 64 KB → 完整 BLAKE3 哈希，多线程），忽略硬链接，按时间、路径或文件夹自动勾选多余副本，重新校验后移到回收站 |
